...
local function translate_input(msg)
    ...
    if msg.type == qtfb.MESSAGE_USERINPUT then
    ...
    elseif msg.type == qtfb.MESSAGE_DEVICE_STATE_CHANGED or msg.type == qtfb.MESSAGE_DEVICE_STATE_INIT then
        if msg.deviceStateChanged.reason == qtfb.STATE_CHANGED_REASON_ROTATION then
            local rotation = msg.deviceStateChanged.rotation.rotation
            if rotation == qtfb.ROTATION_0 then
                emit(C.EV_MSC, C.MSC_GYRO, C.DEVICE_ROTATED_UPRIGHT)
            elseif rotation == qtfb.ROTATION_L90 then
                emit(C.EV_MSC, C.MSC_GYRO, C.DEVICE_ROTATED_COUNTER_CLOCKWISE)
            elseif rotation == qtfb.ROTATION_R90 then
                emit(C.EV_MSC, C.MSC_GYRO, C.DEVICE_ROTATED_CLOCKWISE)
            elseif rotation == qtfb.ROTATION_180 then
                emit(C.EV_MSC, C.MSC_GYRO, C.DEVICE_ROTATED_UPSIDE_DOWN)
            end
        end
    end
